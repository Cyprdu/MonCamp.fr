...
try {
    if ($token) {
        ...
        $res = $stmt->fetch();
        if ($res) {
            // Vérification si camp privé et non admin
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 relative">
            ...
            <div class="lg:col-span-2 space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="bg-white rounded-3xl p-2 shadow-sm border border-gray-100 overflow-hidden">
                        <iframe id="map-frame" class="w-full h-full min-h-[200px] rounded-2xl" frameborder="0" scrolling="no" marginheight="0" marginwidth="0" style="border:0" loading="lazy"></iframe>
                    </div>
                </div>
            </div>
            ...
            <div class="lg:col-span-1">
                <div class="sticky top-[100px] space-y-6">
                    ...
                    <div class="bg-gray-50 rounded-2xl p-2 text-center border border-gray-200 min-h-[250px] relative overflow-hidden">
                        <span class="absolute top-1 right-2 text-[8px] text-gray-400 uppercase bg-gray-200/50 px-1 rounded z-10">Publicité</span>
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    try {
        ...
        if (camp.lien_externe) {
            ctaHtml = `
                <a href="${camp.lien_externe}" target="_blank" class="flex items-center justify-center gap-2 w-full bg-[#0A112F] hover:bg-opacity-90 text-white font-bold py-4 px-6 rounded-xl shadow-lg transform transition hover:scale-[1.02] text-center text-lg">
            ...
            if(isLoggedIn) {
                ctaHtml = `
                    <a href="inscription?camp_t=${camp.token}" class="flex items-center justify-center gap-2 w-full bg-[#0A112F] hover:bg-opacity-90 text-white font-bold py-4 px-6 rounded-xl shadow-lg transform transition hover:scale-[1.02] text-center text-lg">
                        Réserver via ColoMap
                    </a>`;
            } else {
                ctaHtml = `
                    <a href="login.php?redirect=${encodeURIComponent(window.location.href)}" class="flex items-center justify-center gap-2 w-full bg-gray-900 hover:bg-gray-800 text-white font-bold py-4 px-6 rounded-xl shadow-lg transform transition hover:scale-[1.02] text-center text-lg">
        ...
        if(camp.organisateur_user_id) {
            if(isLoggedIn) {
                ...
                loginLink.className = "text-sm text-gray-500 hover:text-[#0A112F] underline text-center block";
                loginLink.textContent = "Connectez-vous pour contacter";
                document.getElementById('contact-container').appendChild(loginLink);
            }
        }
