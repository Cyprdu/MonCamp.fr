...
// 1. SCRIPT TINYMCE OFFICIEL (AVEC TA CLÉ API)
?>
<script src="https://cdn.tiny.cloud/1/4f4fzy1bzi6haifjxlg0vxdubekxl6d76nwq6t29nsct0wn5/tinymce/6/tinymce.min.js" referrerpolicy="origin"></script>
<script>
  tinymce.init({
    selector: '.rich-editor', // Cible le textarea
    height: 300,
    menubar: false,
    plugins: 'anchor autolink charmap codesample emoticons image link lists media searchreplace table visualblocks wordcount',
    toolbar: 'undo redo | blocks fontfamily fontsize | bold italic underline strikethrough | link image media table | align lineheight | numlist bullist indent outdent | emoticons charmap | removeformat',
    content_style: 'body { font-family:Inter,sans-serif; font-size:14px }',
    language: 'fr_FR' // Assure-toi que la langue est configurée dans ton dashboard TinyMCE si besoin
  });
</script>
<?php

// 2. FONCTION EMAIL (NOTIFICATION UNIQUEMENT - PAS DE CONTENU)
function sendNotificationEmail($toEmail, $toName, $question, $token) {
    ...
    <body>
        <div class='container'>
            ...
            <div class='content'>
                ...
                <p>Notre équipe vient de répondre à votre demande concernant :</p>
                
                <div class='question-box'>
                    \"" . nl2br(htmlspecialchars(substr($question, 0, 150))) . "...\"
                </div>

                <p style='margin-bottom: 25px;'>Pour garantir la sécurité et la mise en forme, la réponse est disponible sur notre site.</p>

                <a href='$link' class='btn'>Consulter la réponse</a>
                
                <p style='margin-top: 30px; font-size: 13px; color: #6b7280;'>Lien direct : <a href='$link' style='color:#6b7280;'>$link</a></p>
            </div>
            <div class='footer'>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['reply_id'])) {
    ...
    // TinyMCE envoie le contenu HTML ici
    $reponse = $_POST['reponse']; 
    
    if (!empty($reponse)) {
        // On sauvegarde le HTML en base
        ...
        if($stmt->execute([$reponse, $msg_id])) {
            ...
            if($msgData) {
                // On envoie le mail SANS le contenu HTML (juste le lien)
                $sent = sendNotificationEmail(
                    $msgData['email'], 
                    $msgData['prenom'], 
                    $msgData['message'], 
                    $msgData['token']
                ...
                else $error = "Réponse enregistrée, mais erreur d'envoi mail.";
            }
        } else {
            $error = "Erreur BDD.";
        }
                ...
                <div class="bg-white rounded-xl shadow-sm p-6 border-l-4 <?php echo $view === 'pending' ? 'border-orange-500' : 'border-green-500'; ?>">
                    ...
                    <?php if ($view === 'pending'): ?>
                        <form method="POST" class="mt-4 bg-blue-50 p-4 rounded-lg">
                            ...
                            <label class="block text-sm font-bold text-blue-900 mb-2">Votre réponse (HTML Rich Text) :</label>
                            <textarea name="reponse" class="rich-editor w-full border border-blue-200 rounded-lg p-3 mb-3"></textarea>
                            <button type="submit" class="bg-blue-600 text-white font-bold py-2 px-6 rounded-lg hover:bg-blue-700 mt-2">Envoyer et Archiver</button>
                        </form>
                    <?php else: ?>
