...
<style>
    ...
    body {
        background-color: #FBF6E9; /* Fond Principal */
        color: #4A4A4A; /* Texte Principal */
        font-family: 'Montserrat', sans-serif; /* Police */
    }
    /* Surcharge des styles globaux si nécessaire pour les cartes/blocs principaux */
    .card-block {
        background-color: #FFFFFF; /* Fond des Cartes */
        box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -2px rgba(0, 0, 0, 0.06); /* Ombre douce */
    }
    .text-accent-primary {
        color: #CE3E57; /* Couleur d'Accent 1 */
    }
    .bg-accent-primary {
        background-color: #CE3E57; /* Couleur d'Accent 1 */
    }
    .border-neutral {
        border-color: #B7B4B2; /* Couleur Neutre pour les bordures */
    }
    .loader {
        /* Conserver le style du loader existant ou utiliser une version plus simple */
        border: 4px solid #f3f3f3;
        border-top: 4px solid #CE3E57; /* Couleur d'Accent */
        border-radius: 50%;
        width: 40px;
        height: 40px;
...
<?php if (!$is_animateur): ?>
<div id="filter-bar" class="hidden sticky top-[64px] z-30 bg-white shadow-md transition-all duration-300 card-block rounded-none">
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
        <form id="advanced-search-form" class="py-4">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                
                <select id="filter-department" name="department" class="p-3 border border-neutral rounded-lg focus:ring-[#CE3E57] focus:border-[#CE3E57] text-[#4A4A4A]">
                ...
                <?php if ($is_logged_in): ?>
                    <select id="filter-child" name="age" class="p-3 border border-neutral rounded-lg focus:ring-[#CE3E57] focus:border-[#CE3E57] text-[#4A4A4A]">
                        <option value="">Pour quel enfant ?</option>
                    </select>
                <?php else: ?>
                    <input type="number" id="filter-age" name="age" placeholder="Âge de l'enfant" class="p-3 border border-neutral rounded-lg text-[#4A4A4A]">
                <?php endif; ?>

                <button type="submit" class="w-full bg-accent-primary text-white font-bold rounded-lg hover:bg-[#CE3E57]/90 transition-colors">Rechercher</button>
            </div>
        </form>
    </div>
</div>
...
<main class="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
    ...
    <section class="text-left my-16">
        <?php if ($is_logged_in): ?>
            <h1 class="text-4xl md:text-5xl font-bold tracking-tight text-[#4A4A4A]">
                Bonjour <span class="text-accent-primary"><?php echo htmlspecialchars($_SESSION['user']['prenom']); ?></span>
            </h1>
            <p class="mt-4 max-w-3xl text-xl text-[#4A4A4A]">
        ...
        <?php else: ?>
             <h1 class="text-4xl md:text-5xl font-bold tracking-tight text-[#4A4A4A]">
                Bienvenu sur <span class="text-accent-primary">ColoMap</span>
            </h1>
            <p class="mt-4 max-w-3xl text-xl text-[#4A4A4A]">
    ...
    <section class="mb-12">
        <div class="relative card-block rounded-xl shadow-md">
            <div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-4">
                <svg class="h-5 w-5 text-[#4A4A4A]" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M9 3.5a5.5 5.5 0 100 11 5.5 5.5 0 000-11zM2 9a7 7 0 1112.452 4.391l3.328 3.329a.75.75 0 11-1.06 1.06l-3.329-3.328A7 7 0 012 9z" clip-rule="evenodd" /></svg>
            </div>
            <input type="text" id="name-search-input" placeholder="Rechercher un camp par nom..." class="block w-full rounded-xl border border-neutral p-4 pl-12 text-base text-[#4A4A4A] focus:border-[#CE3E57] focus:ring-[#CE3E57] bg-white">
        </div>
    </section>

    <section class="mt-8">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-bold text-[#4A4A4A]"><?php echo $is_animateur ? "Camps qui recrutent" : "Tous les camps disponibles"; ?></h2>
            <?php if (!$is_animateur): ?>
            <button id="toggle-filters-button" class="inline-flex items-center gap-2 rounded-lg bg-white px-4 py-2 text-sm font-medium text-accent-primary shadow-sm ring-1 ring-inset ring-neutral hover:bg-neutral/10 transition-all border border-accent-primary">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-5 h-5 text-accent-primary"><path fill-rule="evenodd" d="M2.628 1.601C5.028 1.206 7.49 1 10 1s4.973.206 7.372.601a.75.75 0 0 1 .628.74v2.288a2.25 2.25 0 0 1-.659 1.59l-4.682 4.683a2.25 2.25 0 0 0-.659 1.59v3.037c0 .684-.31 1.33-.844 1.757l-1.937 1.55A.75.75 0 0 1 8 18.25v-5.757a2.25 2.25 0 0 0-.659-1.59L2.659 6.22A2.25 2.25 0 0 1 2 4.629V2.34a.75.75 0 0 1 .628-.74Z" clip-rule="evenodd" /></svg>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function fetchAndDisplayCamps(params = {}) {
        ...
        try {
            ...
            if (camps.error || camps.length === 0) {
                ...
                // Utilisation de la couleur Texte Principal
                campsListContainer.innerHTML = `<p class="text-[#4A4A4A] col-span-full text-center py-10">${message}</p>`;
                return;
            }
            renderCamps(camps);
        } catch (error) {
    ...
    function renderCamps(camps) {
        ...
        camps.forEach(camp => {
            ...
            // Prix en couleur Accent Primaire
            const priceDisplay = !isAnimator ? `<p class="text-accent-primary font-bold text-lg">${camp.prix}€</p>` : '';
            
            newContent += `
                <div class="card-block rounded-xl shadow-md overflow-hidden transform hover:-translate-y-1 transition-transform duration-300 group">
                    <div class="relative cursor-pointer" onclick="window.location.href='${detailPage}?${linkParam}'">
                        ...
                        <button class="favorite-button absolute top-3 right-3 bg-white/70 backdrop-blur-sm p-2 rounded-full transition-all hover:scale-110" data-camp-id="${camp.id}" title="Ajouter aux favoris">
                            <svg class="w-5 h-5 ${isFavorited ? 'text-accent-primary fill-current' : 'text-[#4A4A4A]'}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="p-4 cursor-pointer" onclick="window.location.href='${detailPage}?${linkParam}'">
                         <h3 class="font-bold text-lg mb-2 truncate text-[#4A4A4A]">${camp.nom}</h3>
                         <p class="text-[#4A4A4A] text-sm mb-1">📍 ${camp.ville}</p>
                         <p class="text-[#4A4A4A] text-sm mb-3">🎂 ${camp.age_min} - ${camp.age_max} ans</p>
                         <div class="flex justify-between items-center">
                             ${priceDisplay}
                             <span class="text-xs font-semibold text-[#4A4A4A] ml-auto">${new Date(camp.date_debut).toLocaleDateString('fr-FR')}</span>
                         </div>
                    </div>
                </div>`;
        });
        ...
        try {
            ...
            if (result.success) {
                if (result.isFavorited) {
                    ...
                    // Remplacer text-red-500 par text-accent-primary
                    svg.classList.add('text-accent-primary', 'fill-current');
                } else {
                    userFavorites = userFavorites.filter(id => id !== campId);
                    svg.classList.remove('text-accent-primary', 'fill-current');
                }
            }
        } catch (error) {
